...
export const ReceiptDateContainer = styled.div`
 width: 160px ;
 margin-bottom: 16px;
...
export const InputWrapper = styled.div<InputWrapperProps>`
  display: grid;
  grid-template-columns: ${({minmax}) => `repeat(auto-fill, minmax(${minmax}0%, 1fr));`};
  gap: 10px;
  align-items: center;
`

export const ImageShowContianer = styled.div`
  display: flex;
  flex-wrap: wrap;
  gap: 10px;
