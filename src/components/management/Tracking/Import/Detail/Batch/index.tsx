...
import { StockMovementsSnapshot } from "contracts/management";
import { date } from "utils";
import { useSelector } from "react-redux";
import { RootState } from "store";

type BatchDetailProps = {
  snapshot: StockMovementsSnapshot
...
export const BatchDetail = React.memo(
  ({ snapshot, plantDelivery }: BatchDetailProps): JSX.Element => {
    ...
    const {regularizedAt, wsmImportDate, sapImportDate, createdAt, stockElement , wmsQty, sapQty, divergencyQty} = snapshot
    ...
    return (
      <S.Container>
        <S.Title>
          <p>{t("management.tracking.detalhe.lote")}</p>{stockElement?.batch}
        </S.Title>
        <S.Content>
          <S.Info>
            <S.Header>
