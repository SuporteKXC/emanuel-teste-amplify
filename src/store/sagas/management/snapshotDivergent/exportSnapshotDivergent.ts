...
import {
  DivergentExportRequestAction,
  SnapshotDivergentExportActions,
...
function* onProgress(channel: EventChannel<any>): any {
  while (true) {
    ...
    yield put(SnapshotDivergentExportActions.load(Math.floor(loaded)));
  }
}

export function* exportSnapshotDivergentRequest(action: any): any {
  const { query, onSuccess, onFailure }: DivergentExportRequestAction = action;
  try {
    ...
    yield all([
      put(SnapshotDivergentExportActions.success(data.data, data.meta)),
    ]);
  } catch (error) {
    ...
    console.log(error);
    yield put(SnapshotDivergentExportActions.failure(errorMessage));
  }
}
