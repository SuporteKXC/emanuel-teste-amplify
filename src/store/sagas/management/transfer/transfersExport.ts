...
import { TransfersExportRequestAction, TransfersExportActions } from "store/ducks/management/transfer";
...
function* onProgress(channel: EventChannel<any>): any {
  while (true) {
    ...
    yield put(TransfersExportActions.load(Math.floor(loaded)));
  }
}

export function* exportTransfersRequest(action: any): any {
  const { query, onSuccess, onFailure }: TransfersExportRequestAction = action;
  ...
  try {
    ...
    yield all([put(TransfersExportActions.success(data.data, data.meta))]);
  } catch (error) {
    const { errorMessage } = apiErrorHandler(error);
    onFailure && onFailure();
    notify("error", errorMessage);
    console.log(error);
    yield put(TransfersExportActions.failure(errorMessage));
  }
}
