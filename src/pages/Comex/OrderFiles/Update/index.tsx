...
import {
  ...
  ShowOrderFilesActions,
  UpdateOrderFileActions,
} from "@/store/ducks";
...
export const UpdateOrderFile = () => {
  ...
  const { loading } = useSelector((state: RootState) => state.updateOrderFile);
  const { data: orders, loading: ordersLoading } = useSelector(
    (state: RootState) => state.searchOrderOptions
  );
  const { data: orderFile, loading: orderFileLoading } = useSelector(
    (state: RootState) => state.showOrderFiles
  );
  ...
  const fetch = () => {
    ...
    dispatch(ShowOrderFilesActions.request(id));
  };
  const form = useForm({
  ...
  const handleSubmit = (data: FormData) => {
    const validData: any = data.links.map((link, index) => {
      return {
        order_id: data.orderId,
        description: data.description,
        file_link: link.fileLinks,
      };
    });
    for (const index of data.links.keys()) {
      if (orderFile?.order.order_files[index]) {
        validData[index].id = `${orderFile?.order.order_files[index].id}`;
      }
    }
    console.log(validData);

    dispatch(
      UpdateOrderFileActions.request(id, { files: validData }, onSuccess)
    );
  };
  ...
  useEffect(() => {
    if (orderFile) {
      form.setValue("orderId", `${orderFile.order_id}`);
      form.setValue("description", orderFile.description);
      form.setValue(
        "links",
        orderFile.order.order_files.map((item) => ({
          fileLinks: item.file_link,
        }))
      );
    }
  }, [orderFile, orders]);
          ...
          <FormField
            ...
            render={({ field }) => (
              <>
                ...
                <TextareaGs
                  className="w-full"
                  placeholder="Adicione a descrição da documentação..."
                  {...field}
                />
              </>
            )}
