...
import { PaginateCompanyMembersActions as PaginateActions } from 'store/ducks/companyMembers';
import * as S from './styles';

export const CompanyMembersListPage: React.FC = () => {
  ...
  const { paginationCache, updatePaginationCache, handleFilter, handleSort } =
    usePaginationCache<FindMembers>('companyMembers');

  const {
    data: members,
    pagination,
    loading,
  } = useSelector((state: RootState) => state.paginateCompanyMembers);
    ...
    <Scaffold>
      ...
      <S.MainPanel>
        <ListingPageHeader
          ...
          actions={
            <S.LinkButton
              to={'/configuracoes/clientes/usuarios/criar'}
              size="small"
            >
