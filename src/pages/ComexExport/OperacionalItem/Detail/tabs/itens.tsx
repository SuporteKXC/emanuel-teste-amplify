...
import { ExportOrderItem } from "@/contracts/exportOrder/exportOrder";
...
const Items: React.FC<{ order: ExportOrderItem }> = ({ order }) => {
  ...
  const infoItems = useMemo(
    () => [
      {
        title: "PO",
        value: `${order?.exportOrder?.order_reference} - ${order.item}` || "",
      },
      { title: "Customer PO", value: order?.exportOrder?.customer_po || "" },
      { title: "Produto", value: order?.product?.description || "" },
      { title: "Quantidade", value: Number(order?.qty).toFixed(2) },
    ],
    [order]
  );

  const { data: exportOrderJustificationData } = useSelector(
    (state: RootState) => state.getExportOrderJustification
  );

  const fetchExportOrderJustifications = useCallback(() => {
    if (order?.exportOrder?.id) {
      dispatch(
        GetExportOrderJustificationActions.request({
          exportOrder: order?.exportOrder?.id,
        })
      );
    }
  }, [order?.exportOrder?.id]);
  useEffect(() => {
  ...
  const handleSubmit = (data: FormData) => {
    const validData = {
      ...
      export_order_id: order?.exportOrder?.id,
    };
    ...
    <div className="flex flex-col gap-4 h-full">
      <section className="w-full px-4 pb-4 border-b-4 border-green-400 justify-between flex bg-white rounded">
        {infoItems.map((item, index) => (
          <div key={index}>
            <h2 className="font-GilroyBold">{item.title}</h2>
            <p className="font-GilroyRegular">{item.value}</p>
          </div>
        ))}
      </section>
        ...
        <InfoCard
          ...
          content={
            <>
              ...
              <p className="font-semibold text-sm text-gray-900">Modal:</p>
              <p className="text-xs text-gray-500">{order?.modal}</p>

              <p className="font-semibold text-sm text-gray-900">
                Tipo de processo:
              </p>
              <p className="text-xs text-gray-500">{order?.process_type}</p>

              <p className="font-semibold text-sm text-gray-900">
                Analista Responsável:
              </p>
              <p className="text-xs text-gray-500">
                {order?.exportOrder.customer_po}
              </p>
            </>
          }
        />
        <InfoCard
          ...
          content={
            <>
              ...
              <p className="text-xs text-gray-500">
                {order?.exportOrder.company?.nameFantasy}
              </p>
              <p className="font-semibold text-sm text-gray-900">Origem:</p>
              <p className="text-xs text-gray-500">
                {order?.exportOrder?.company?.country}
              </p>
            </>
          }
        />
        ...
        <InfoCard
          ...
          content={
            <>
              ...
              <p className="text-xs text-gray-500">{order?.port_clearance}</p>
              <p className="font-semibold text-sm text-gray-900">Destino:</p>
        ...
        <InfoCard
          ...
          content={
            <>
              ...
              <p className="text-xs text-gray-500">
                {order?.etd_date && Formatter.nativeFormat(order?.etd_date)}
              </p>
              <p className="font-semibold text-sm text-gray-900">ETA:</p>
              <p className="text-xs text-gray-500">
                {order?.eta_date && Formatter.nativeFormat(order?.eta_date)}
              </p>
              <p className="font-semibold text-sm text-gray-900">
                ETA Revisado:
              </p>
              ...
              <p className="text-xs text-gray-500">
                {order?.etb_date && Formatter.nativeFormat(order?.etb_date)}
              </p>
              <p className="font-semibold text-sm text-gray-900">GR Atual:</p>
              <p className="text-xs text-gray-500">
                {order?.gr_actual && Formatter.nativeFormat(order?.gr_actual)}
              </p>
            </>
          }
        />
        ...
        <div className="w-full flex justify-between">
          ...
          {exportOrderJustificationData.length == 0 && (
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent>
                <JustificationForm handleSubmit={handleSubmit} isOpen={open} />
              </DialogContent>
            </Dialog>
          )}
        </div>
