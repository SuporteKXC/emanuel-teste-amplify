...
import { ClientDocumentType } from "contracts";
...
export const getClientShape = (props: ClientProps) => {
  const { documentType, addressCountry, docType } = props;
  ...
  const documentNumberSchema = !docType
    ? Yup.mixed().test({
        name: "testDocument",
    ...
    : Yup.string().required("Informe o documento");

  return {
    documentType: Yup.string().required("Informe o tipo de cliente"),
    documentNumber: documentNumberSchema,
    email: Yup.array().required("Email é obrigatório").min(1),
    tradeName: Yup.string().required((): string => {
      return documentType === "cnpj"
        ? "Informe a razão social"
        : "Informe o nome completo";
    }),
    days: Yup.array().nullable().optional(),
...
export const getClientLogoShape = () => {
  return {
